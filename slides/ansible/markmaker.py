...
def generatefromyaml(manifest, filename):
    ...
    html = open("training.html").read()
    html = html.replace("@@MARKDOWN@@", markdown)
    html = html.replace("@@EXCLUDE@@", exclude)
    html = html.replace("@@CHAT@@", manifest["chat"])
    html = html.replace("@@TRAINER@@", manifest["trainer"])
    html = html.replace("@@TRAINER_PROFILE@@", manifest["trainer_profile"])
    html = html.replace("@@SLIDES@@", manifest["slides"])
    html = html.replace("@@SLIDES_REPOS@@", manifest["slides_repo"])
    html = html.replace("@@LAB@@", manifest["lab_playground"])
